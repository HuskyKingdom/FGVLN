...
def load_dataloader(args, default_gpu, logger, local_rank, model) -> str:
    ...
    if not args.no_test and not args.pretrain:
        val_seen_dataset = load_BeamDataset("val_seen", args, tokenizer, features_reader, default_gpu, model, Train=False)
        val_unseen_dataset = load_BeamDataset("val_unseen", args, tokenizer, features_reader, default_gpu, model, Train=False) # CHANGE~
    ...
    print(f" train {np.random.choice(range(len(train_dataset)), size=850, replace=False)} \n val_seen_dataset {len(val_seen_dataset)} \n val_unseen_dataset {len(val_unseen_dataset)}")
    if args.mini:
            # np.random.seed(0) # fixed data
            train_dataset = Subset(
                train_dataset,
                np.random.choice(range(len(train_dataset)), size=850, replace=False),  # type: ignore 850
            )
            val_seen_dataset = Subset(
                val_seen_dataset,
                np.random.choice(range(len(val_seen_dataset)), size=120, replace=False),  # type: ignore 120
            )
            val_unseen_dataset = Subset(
                val_unseen_dataset,
                np.random.choice(range(len(val_unseen_dataset)), size=120, replace=False),  # type: ignore
            )
